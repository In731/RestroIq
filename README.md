# RestroIQ — AI-Powered Restaurant Revenue Intelligence Platform
...
2. Twilio hits POST /twilio/voice
   → Server generates TwiML: "Welcome to RestroIQ! What would you like to order?"
...
Each visitor gets a persistent UUID stored in `localStorage` under the key `restroiq_session` (generated via `crypto.randomUUID()`). This session ID:
...
This project is proprietary and developed as a final competition project for the RestroIQ Revenue Intelligence platform. All rights reserved.
